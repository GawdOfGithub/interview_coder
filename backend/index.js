...
// const Question = require('./models/Question'); // Remove Question model import
...
app.post('/parse-resume', upload.single('resume'), async (req, res) => {
    ...
    try {
        ...
        let candidate;
        try {
            candidate = new Candidate({ name: extractedName, email: extractedEmail, phone: extractedPhone });
...
app.post('/submit-quiz', async (req, res) => {
    ...
    console.log('Received quiz submission:', { candidateId, score }); // Add logging

    if (!candidateId || score === undefined) {
        return res.status(400).json({ error: 'Candidate ID and score are required.' });
    }

    try {
        const newScore = new Score({ candidateId, score });
        const savedScore = await newScore.save(); // Capture the saved document
        console.log('Score saved to MongoDB:', savedScore); // Log the saved document
...
app.get('/scores/:candidateId', async (req, res) => {
    ...
    try {
        ...
        if (scores.length === 0) {
            console.warn('Backend: No scores found for candidate ID:', candidateId); // New debug log

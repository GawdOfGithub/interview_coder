...
// --- Remove New Mock Data with Difficulty and Timers ---
// const quizData = {
//   questions: [
//     // Easy Questions
...
export default function QuestionPage() {
    ...
    const [quizQuestions, setQuizQuestions] = useState([]); // New state for fetched questions
    const [loadingQuestions, setLoadingQuestions] = useState(true); // New state for loading status
  
    const currentQuestion = quizQuestions[currentQuestionIndex]; // Use fetched questions
    const [timeLeft, setTimeLeft] = useState(currentQuestion ? currentQuestion.timeLimit : 0); // Initialize with 0 or a default
  
    // --- Fetch Questions from Backend ---
    useEffect(() => {
        const fetchQuestions = async () => {
            try {
                const response = await fetch('http://localhost:3000/questions?limit=5'); // Fetch 5 random questions
                const data = await response.json();
                setQuizQuestions(data.map(q => ({ ...q, timeLimit: 60 }))); // Assign a default timeLimit for now
            } catch (error) {
    ...
    const handleNextStep = useCallback(() => {
      const isLastQuestion = currentQuestionIndex === quizQuestions.length - 1; // Use fetched questions length
      if (isLastQuestion) {
    ...
    const handleSubmit = useCallback(() => {
        if (!currentQuestion) return; // Prevent submission if no question is loaded
        ...
        setUserAnswers(prevAnswers => [
          ...prevAnswers,
          { question: currentQuestion.question, answer: selectedOption, isCorrect, timeTaken }
        ]);
        
        handleNextStep();
    }, [selectedOption, currentQuestion, timeLeft, handleNextStep]);
  
    // --- Effects for Timer Management ---
    useEffect(() => {
      if (quizFinished || loadingQuestions || !currentQuestion) return; // Add loadingQuestions and currentQuestion check
    ...
    }, [quizFinished, loadingQuestions, currentQuestion]); // Add loadingQuestions and currentQuestion to dependencies
    
    useEffect(() => {
        if (!quizFinished && timeLeft <= 0 && !loadingQuestions) {
            handleSubmit(); // Auto-submit when time runs out
        }
    }, [timeLeft, quizFinished, handleSubmit, loadingQuestions]); // Add loadingQuestions to dependencies
  
    useEffect(() => {
      if (!quizFinished && currentQuestion) { // Check if currentQuestion exists
        setTimeLeft(currentQuestion.timeLimit);
      }
    }, [currentQuestionIndex, quizFinished, currentQuestion]); // Add currentQuestion to dependencies

    // --- Derived State for Performance Metrics & Summary ---
    const performance = useMemo(() => {
      if (!quizFinished || quizQuestions.length === 0) return null; // Use fetched questions length
  
      const accuracy = (score / quizQuestions.length) * 100; // Use fetched questions length
      const totalTimeTaken = userAnswers.reduce((acc, ans) => acc + ans.timeTaken, 0);
      const avgTime = quizQuestions.length > 0 ? totalTimeTaken / quizQuestions.length : 0; // Use fetched questions length
  
      let summaryText, summaryColor;
      if (accuracy >= 80) { // High performance (5-6 correct)
        summaryText = "High Performance! You have a strong grasp of the concepts.";
        summaryColor = "text-green-400";
      } else if (accuracy >= 50) { // Average performance (3-4 correct)
        summaryText = "Average Performance. Good effort, with room for improvement.";
        summaryColor = "text-yellow-400";
      } else { // Poor performance (0-2 correct)
    ...
    }, [quizFinished, score, userAnswers, quizQuestions.length]); // Add quizQuestions.length to dependencies

    // --- Redux Integration & Backend Score Submission ---
    ...
    useEffect(() => {
        const submitScore = async () => {
            if (quizFinished && performance && currentCandidateId) {
                console.log('QuestionPage: currentCandidateId before submission:', currentCandidateId); // Debugging
                console.log('Submitting score to backend:', { candidateId: currentCandidateId, score: parseFloat(performance.accuracy.toFixed(0)) });
                try {
                    ...
                    if (response.ok) {
                        console.log('Score submitted successfully to backend:', result);
                        dispatch(updateCandidateScore({ candidateId: currentCandidateId, scoreValue: parseFloat(performance.accuracy.toFixed(0)) }));

                        // --- Generate AI Summary --- //
                        console.log('Generating AI summary for candidate:', currentCandidateId);
                        ...
                        if (summaryResponse.ok) {
                            console.log('AI summary generated and saved:', summaryResult.aiSummary);
        ...
        submitScore();
    }, [quizFinished, performance, currentCandidateId, dispatch]);
    ...
    const handleTryAgain = () => {
      ...
      setScore(0);
      setUserAnswers([]);
      // Reset time for the first question fetched from backend
    ...
    if (quizQuestions.length === 0) {
        return <div className="flex items-center justify-center min-h-screen bg-slate-900 text-white">No quiz questions available. Please seed the database.</div>;
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-900 font-sans p-4 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 -right-1/4 w-96 h-96 bg-purple-600/50 rounded-full filter blur-3xl opacity-20 animate-pulse animation-delay-4000"></div>
        ...
        <div className="w-full max-w-6xl bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl shadow-2xl p-8 grid md:grid-cols-2 gap-12 z-10">
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {currentQuestion.options.map((option, index) => (
                    <button key={index} onClick={() => setSelectedOption(option)} className={`p-4 rounded-lg text-left transition-all duration-300 border-2 ${selectedOption === option ? 'bg-blue-600 border-blue-500 text-white shadow-lg' : 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700 hover:border-slate-500'}`}>
          ...
          <div className="flex flex-col items-center justify-between bg-slate-900/70 p-6 rounded-xl border border-slate-700">
            ...
            {!quizFinished && !loadingQuestions && currentQuestion ? <CircularTimer timeLeft={timeLeft} totalTime={currentQuestion.timeLimit} /> : quizFinished ? <div className="text-green-400 text-center my-auto"><h3 className="text-2xl font-bold">Finished!</h3></div> : null} {/* Add loadingQuestions and currentQuestion check */}
